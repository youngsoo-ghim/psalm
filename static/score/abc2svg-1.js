...
abc2svg.Abc = function(user) {
        ...
        for (i = 0; i < nd; i++) {
            ...
            switch (dd.func) {
                ...
                case 33:
                    ...
                    continue;
                case 34:
                    if (s.type != NOTE || !prev || prev.type != NOTE || s.nflags != prev.nflags) {
    ...
    function draw_deco_near() {
        ...
        function ll_deco() {
            ...
            for (i = 0; i < n_de; i++) {
                ...
                if (dd.name == "trill(" && i > 0 && a_de[i - 1].dd.name == "trill") de2.prev = a_de[i - 1]
            }
            for (i = 0; i < n_de; i++) {
                de2 = a_de[i];
    ...
    function draw_measnb() {
        ...
        if (bar_num > 1) {
            if (cfmt.measurenb == 0) {
                ...
                if (cfmt.measurebox) xy_str_b(0, y, bar_num.toString());
                else xy_str(0, y, bar_num.toString());
    ...
    function calculate_beam(bm, s1) {
        var s, s2, notes, nflags, st, v, two_staves, two_dir, x, y, ys, a, b, stem_err, max_stem_err, p_min, p_max, s_closest, stem_xoff, scale, visible, dy;
        if (!s1.beam_st) {
            s = sym_dup(s1);
            lkvsym(s, s1);
            lktsym(s, s1);
    ...
    function draw_beams(bm) {
        ...
        function draw_beam(x1, x2, dy, h, bm, n) {
            ...
            xypath(x1, y1, true);
            output += "l" + x2.toFixed(2) + " " + (-dy2).toFixed(2) + "v" + h.toFixed(2) + "l" + (-x2).toFixed(2) + " " + dy2.toFixed(2) + 'z"/>\n'
        }
        anno_start(s1, "beam");
        if (!s1.grace) {
            bshift = BEAM_SHIFT;
    ...
    function draw_meter(x, s) {
        if (!s.a_meter) return;
        var dx, i, j, tmp1, tmp2, st = s.st,
            p_staff = staff_tb[st],
            y = p_staff.y;
        if (p_staff.stafflines != "|||||") y += (p_staff.topbar + p_staff.botbar) / 2 - 12;
        for (i = 0; i < s.a_meter.length; i++) {
            var f, meter = s.a_meter[i];
            x = s.x + s.x_meter[i];
            if (meter.bot) {
                tmp1 = tmp2 = "";
                for (j = 0; j < meter.top.length; j++) tmp1 += tgls["meter" + meter.top[j]].c;
                for (j = 0; j < meter.bot.length; j++) tmp2 += tgls["meter" + meter.bot[j]].c;
    ...
    function draw_hl(x, yl, yu, st, hltype) {
        ...
        if (yl % 6) yl += 3;
        if (yu % 6) yu -= 3;
        if (stafflines.indexOf("-") >= 0 && (yl > bot && yl < top || yu > bot && yu < top || yl <= bot && yu >= top)) {
            i = yl;
            j = yu;
        ...
        for (; yl < bot; yl += 6) xygl(x, staffb + yl, hltype);
    ...
    function draw_gracenotes(s) {
        ...
        output += "c" + x1.toFixed(2) + " " + (-y1).toFixed(2) + " " + x2.toFixed(2) + " " + (-y2).toFixed(2) + " " + (x3 - x0).toFixed(2) + " " + (-y3 + y0).toFixed(2) + '"/>\n';
        anno_stop(s, "slur")
    }
    ...
    function draw_basic_note(x, s, m, y_tb) {
        var i, k, p, yy, dotx, doty, old_color = false,
            note = s.notes[m],
    ...
    function slur_out(x1, y1, x2, y2, dir, height, dotted) {
        ...
        if (!dotted) output += '<path class="fill" d="M';
        else output += '<path class="stroke" stroke-dasharray="5,5" d="M';
        out_sxsy(x1, " ", y1);
        output += "c" + ((xx1 - x1) / stv_g.scale).toFixed(2) + " " + ((y1 - yy1) / scale_y).toFixed(2) + " " + ((xx2 - x1) / stv_g.scale).toFixed(2) + " " + ((y1 - yy2) / scale_y).toFixed(2) + " " + ((x2 - x1) / stv_g.scale).toFixed(2) + " " + ((y1 - y2) / scale_y).toFixed(2);
    ...
    function draw_tuplet(s1, lvl) {
        ...
        out_tubrn(x1, y1, x2 - x1, y2 - y1, dir == SL_ABOVE, s1.tf[2] == 0 ? p.toString() : p + ":" + q);
        yy = .5 * (y1 + y2);
        if (dir == SL_ABOVE) y_set(upstaff, true, xm - 3, 6, yy + 9);
            ...
            switch (s.type) {
                case STAVES:
                    ...
                    for (st = 0; st <= nstaff; st++) {
                        x = xstaff[st];
                        if (x < 0) {
                            if (cur_sy.st_print[st]) xstaff[st] = s.ts_next.type == BAR ? s.x : s.x - s.wl - 2;
                            continue
                        }
                        if (cur_sy.st_print[st]) continue;
                        if (staves_bar) {
                ...
                case BAR:
                    ...
                    if (xstaff[st] < 0) {
                        for (s2 = s.ts_next; s2 && s2.time == s.time; s2 = s2.ts_next) {
                            if (s2.type == STAVES) break
                        }
                        if (!s2 || s2.type != STAVES) break;
                        xstaff[st] = s.x;
                        bar_set()
                    }
    ...
    function tosvg(in_fname, file, bol, eof) {
        var i, c, bol, eol, end, ext, select, line0, line1, last_info, opt, text, a, b, s, cfmt_sav, info_sav, char_tb_sav, glovar_sav, maps_sav, mac_sav, maci_sav, pscom, txt_add = "\n";
        ...
        function uncomment(src, do_escape) {
            if (src.indexOf("%") >= 0) src = src.replace(/(.*[^\\])%.*/, "$1").replace(/\\%/g, "%");
            src = src.replace(/\s+$/, "");
        ...
        if (abc2svg.modules && (abc2svg.modules.hooks.length || abc2svg.modules.g_hooks.length)) set_hooks();
        parse.file = file;
        ...
        for (; bol < eof; bol = parse.eol + 1) {
            ...
            if (pscom) {
                ...
                switch (a[0]) {
                    ...
                    case "abc-include":
                        ext = a[1].match(/.*\.(.*)/);
                        if (ext && ext[1] == "abc") do_include(a[1]);
                        continue
                }
                if (a[0].slice(0, 5) == "begin") {
                    ...
                    if (i < 0) {
                        syntax(1, "No $1 after %%$2", end.slice(1), a[0]);
                        parse.eol = eof;
                        continue
                    }
                    do_begin_end(b, a[1], file.slice(eol + 1, i).replace(new RegExp("^" + line0 + line1, "gm"), ""));
    ...
    var gene, staff_tb, nstaff, tsnext, realwidth, insert_meter, beta_last, space_tb = new Float32Array([7, 10, 14.15, 20, 28.3, 40, 56.6, 80, 100, 120]),
        smallest_duration;
    ...
    function insert_clef(s, clef_type, clef_line) {
        ...
        lktsym(new_s, s);
        if (new_s.ts_prev.type != CLEF) new_s.seqst = true;
        return new_s
    }
    ...
    function set_graceoffs(s) {
        var next, m, dx, gspleft = Number(cfmt.gracespace[0]),
        ...
        switch (s.type) {
            ...
            case REST:
                ...
                for (m = 0; m <= s.nhd; m++) {
                    ...
                    if (s.notes[m].acc) {
                        var tmp = s.notes[m].shac + (s.notes[m].micro ? 5.5 : 3.5);
                ...
                if (s.dots > 0) {
                    if (s.wl == undefined) switch (s.head) {
                        case SQUARE:
                            s.xmx += 4;
                            break;
                        case OVALBARS:
            ...
            case BAR:
                ...
                if (!s.invis) {
                    ...
                    s.wl = w;
                    if (s.next && s.next.type != METER) s.wr = 7;
                    else s.wr = 5;
                    for (s2 = s.prev; s2; s2 = s2.prev) {
                        if (w_tb[s2.type] != 0) {
                            if (s2.type == GRACE) s.wl -= 8;
                            break
                        }
                    }
                } else {
            ...
            case KEY:
                ...
                if (!s.k_a_acc) {
                    ...
                    if (n2) var last_acc = s.k_a_acc[0].acc;
                    for (i = 1; i < n2; i++) {
                        acc = s.k_a_acc[i];
            ...
            case METER:
                ...
                for (i = 0; i < s.a_meter.length; i++) {
                    ...
                    if (meter.top[0] == "C") {
                        ...
                        w = 0;
                        if (!meter.bot || meter.top.length > meter.bot.length) meter = meter.top;
    ...
    function set_repeat(s) {
        ...
        if (n < 0) {
            ...
            for (j = k; --j >= 0;) {
                ...
                s.beam_st = true;
                set_width(s);
                if (s.seqst) s.space = set_space(s);
                s.head = SQUARE;
    ...
    function set_nl(s) {
        ...
        for (;; s = s.ts_next) {
            ...
            switch (s.type) {
                case STAVES:
                    if (s.ts_prev && s.ts_prev.type == BAR) break;
                    while (s.ts_next) {
                        if (w_tb[s.ts_next.type] != 0 && s.ts_next.type != CLEF) break;
                        s = s.ts_next
                    }
    ...
    function set_lines(s, last, lwidth, indent) {
        var first, s2, s3, x, xmin, xmid, xmax, wwidth, shrink, space, nlines, cut_here;
    ...
    function cut_tune(lwidth, indent) {
        ...
        if (cfmt.barsperstaff) {
            i = cfmt.barsperstaff;
            for (s2 = s; s2; s2 = s2.ts_next) {
                if (s2.type != BAR || !s2.bar_num || --i > 0) continue;
                s2.eoln = true;
                i = cfmt.barsperstaff
            }
        }
        ...
        for (; s; s = s.ts_next) {
            ...
            if (xmin > lwidth) {
                ...
                if (s.dur) {
                    ...
                    if (s3 && !s3.seqst) s2 = set_lines(s2, s, lwidth, indent);
                    else s2 = set_nl(s)
                } else {
                    s2 = set_nl(s)
                }
    ...
    function set_clefs() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (!sy.voices[v].second) {
                ...
                if (p_voice.staffnonote != undefined) sy.staves[st].staffnonote = p_voice.staffnonote;
        ...
        for (st = 0; st <= sy.nstaff; st++) mid[st] = (sy.staves[st].stafflines.length - 1) * 3;
        for (s = tsfirst; s; s = s.ts_next) {
        ...
        while (s) {
            ...
            for (; s != u; s = s.ts_next) {
                ...
                switch (s.type) {
                    default: continue;
                    case REST:
                            if (s.combine != undefined && s.combine < 0 || !s.ts_next || s.ts_next.type != REST || s.ts_next.st != s.st || s.time != s.ts_next.time || s.dur != s.ts_next.dur || s.invis) break;unlksym(s.ts_next);
                        break;
                    case NOTE:
                            case GRACE:
                            break
    ...
    function init_music_line() {
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (s2.ts_prev.type != s2.type) s2.seqst = true;
    ...
    function set_indent(first) {
        ...
        if (font) maxw += 4 * cwid(" ") * font.swfac;
        w = .5;
        for (st = 0; st <= cur_sy.nstaff; st++) {
    ...
    function set_piece() {
        var s, last, p_voice, st, v, nst, nv, non_empty = [],
            non_empty_gl = [],
            sy = cur_sy;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            switch (s.type) {
                case CLEF:
                    ...
                    break;
                case BAR:
                    if (!sy.staves[st].staffnonote || sy.staves[st].staffnonote <= 1) break;
                case GRACE:
                ...
                case MREST:
                    if (sy.staves[st].staffnonote > 1) {
                        ...
                        if (sy.staves[st].staffnonote != 0 || s.type == NOTE) non_empty_gl[st] = non_empty[st] = true
                    }
                    break
            }
        }
        ...
        if (last.type != BAR) {
            ...
            s.shrink = last.wr + 2;
            s.space = set_space(s);
            if (s.space < s.shrink && last.type != KEY) s.space = s.shrink
        }
    }
    ...
    function set_sym_glue(width) {
        ...
        var some_grace, s = tsfirst,
            ...
            x = 0;
        while (1) {
            if (s.type == GRACE && !some_grace) some_grace = s;
            if (s.seqst) {
                space = s.space;
                xmin += s.shrink;
        ...
        beta0 = 1;
        if (tsnext && blocks.length == 0 && tsnext.type != BLOCK) {
            if (x >= width) {
                beta_last = 0
            } else {
        ...
        while (1) {
            if (s.seqst) {
                space = s.shrink;
                if (s.space != 0) xmax += s.space * spafac * 1.8;
                x += space;
                xmax += space;
                s.x = x;
                s.xmax = xmax
        ...
        if (x >= width) {
            ...
            if (x == xmin) {
                ...
                alfa = (x - width) / (x - xmin);
                if (alfa > 1) {
                    error(1, s, "Line too much shrunk $1 $2 $3", xmin.toFixed(2), x.toFixed(2), width.toFixed(2))
                }
            }
    ...
    function set_vp(a) {
        ...
        while (1) {
            item = a.shift();
            if (!item) break;
            if (item[item.length - 1] == "=" && a.length == 0) {
            ...
            switch (item) {
                ...
                case "instrument=":
                    ...
                    break;
                case "map=":
                    if (cfmt.sound != "play") curvoice.map = a.shift();
                    break;
                case "name=":
                case "nm=":
                    curvoice.nm = a.shift();
                ...
                case "scale=":
                    ...
                    if (isNaN(val) || val < .6 || val > 1.5) syntax(1, errs.bad_val, "%%voicescale");
    ...
    function new_note(grace, tp_fact) {
        ...
        switch (c) {
            ...
            case "z":
                ...
                line.index++;
                nd = parse_dur(line);
                s.dur_orig = (curvoice.ulen < 0 ? 15120 : curvoice.ulen) * nd[0] / nd[1];
            ...
            default:
                ...
                while (1) {
                    ...
                    note = parse_basic_note(line, s.grace ? BASE_LEN / 4 : curvoice.ulen < 0 ? 15120 : curvoice.ulen);
                    if (!note) return;
        ...
        sym_link(s);
        if (s.type == NOTE) {
            if (curvoice.vtransp) note_transp(s);
    ...
    function parse_music_line() {
        var grace, last_note_sav, a_dcn_sav, no_eol, s, tp_a = [],
            tp, tpn = -1,
            tp_fact = 1,
            slur_start = 0,
            line = parse.line;
                ...
                switch (type[0]) {
                    ...
                    case "(":
                        ...
                        if (c > "0" && c <= "9") {
                            ...
                            if (qplet == 0 || qplet == undefined) qplet = curvoice.wmeasure % 9 == 0 ? 3 : 2;
                            tp = tp_a[++tpn];
                            if (!tp) tp_a[tpn] = tp = {};
                            tp.p = pplet;
                            tp.q = qplet;
    ...
    function write_title(title, is_subtitle) {
        ...
        if (cfmt.titleleft) xy_str(0, 0, title);
        else xy_str(get_lwidth() / 2, 0, title, "c")
    }
    ...
    var glyphs = {
        ...
        sfz: '<text id="sfz" x="-5" y="-7" style="font-family:serif; font-style:italic; font-size:14px">\n\ts<tspan font-size="16" font-weight="bold">f</tspan>z</text>',
    ...
    function out_bar(x, y, h, dotted) {
        output += '<path class="stroke" stroke-width="1" ' + (dotted ? 'stroke-dasharray="5,5" ' : "") + 'd="m' + (x + posx).toFixed(2) + " " + (posy - y).toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n'
    }
    ...
    function out_brace(x, y, h) {
        ...
        y = posy - y;
        h /= 24;
        output += '<text transform="translate(' + x.toFixed(2) + "," + y.toFixed(2) + ") scale(2.5," + h.toFixed(2) + ')">' + tgls.brace.c + "</text>\n"
    }
    ...
    function out_thbar(x, y, h) {
        x += posx + 1.5;
        y = posy - y;
        output += '<path class="stroke" stroke-width="3" d="m' + x.toFixed(2) + " " + y.toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n'
    }
    ...
    function out_tubrn(x, y, dx, dy, up, str) {
        ...
        output += '<path class="stroke" d="m';
        out_sxsy(x, " ", y);
        output += "v" + h.toFixed(2) + "m" + dx.toFixed(2) + " " + (-dy).toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n' + '<path class="stroke" stroke-dasharray="' + ((dx - sw) / 2).toFixed(2) + " " + sw.toFixed(2) + '" d="m';
        out_sxsy(x, " ", y - h);
    ...
    function out_8va(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font-family:serif; font-weight:bold; font-style:italic; font-size:12px">8<tspan dy="-4" style="font-size:10px">va</tspan></text>\n', x - 8, y);
            x += 12;
            val -= 12
        } else {
            val -= 5
    ...
    function out_8vb(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font-family:serif; font-weight:bold; font-style:italic; font-size:12px">8<tspan dy="-4" style="font-size:10px">vb</tspan></text>\n', x - 8, y);
            x += 4;
            val -= 4
        } else {
            val -= 5
    ...
    function out_15ma(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font-family:serif; font-weight:bold; font-style:italic; font-size:12px">15<tspan dy="-4" style="font-size:10px">ma</tspan></text>\n', x - 10, y);
            x += 20;
            val -= 20
        } else {
            val -= 5
    ...
    function out_15mb(x, y, val, defl) {
        if (!defl.nost) {
            out_XYAB('<text x="X" y="Y" style="font-family:serif; font-weight:bold; font-style:italic; font-size:12px">15<tspan dy="-4" style="font-size:10px">mb</tspan></text>\n', x - 10, y);
            x += 7;
            val -= 7
        } else {
            val -= 5
    ...
    function voice_filter() {
        ...
        for (opt in parse.voice_opts) {
            if (!parse.voice_opts.hasOwnProperty(opt)) continue;
            sel = new RegExp(opt);
            if (sel.test(curvoice.id) || sel.test(curvoice.nm)) {
                for (i in parse.voice_opts[opt])
                    if (parse.voice_opts[opt].hasOwnProperty(i)) do_pscom(parse.voice_opts[opt][i])
            }
        }
    }
    ...
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, nb, ir2, v2, sy, nv = voice_tb.length,
            vtb = [],
            vn = [],
            mrest_time = -1;
        ...
        while (1) {
            ...
            for (ir = 0; ir < nv; ir++) {
                ...
                if (s.type == STAVES) {
                    ...
                    for (v2 = 0; v2 < nv; v2++) {
                        if (!new_sy.voices[v2]) continue;
                        ir = new_sy.voices[v2].range;
                        if (ir < 0 || sy.voices[v2].range >= 0) continue;
                        vn[ir2++] = v2
                    }
                }
                if (fl) {
    ...
    function new_syst(init) {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (p_voice.staffnonote != undefined) sy_staff.staffnonote = p_voice.staffnonote;
        ...
        for (; s; s = s.ts_next) {
            switch (s.type) {
                ...
                case BAR:
                    if (s.bar_num) {
                        bar_num = s.bar_num;
                        if (s.time < bar_time) delete s.bar_num
                    }
                    if (s.time < bar_time) {
                        if (s.text && s.text[0] == "1") {
                            bar_rep = bar_num;
    ...
    function set_transp() {
        ...
        if (cfmt.transp && curvoice.transp) syntax(0, "Mix of old and new transposition syntaxes");
    ...
    function get_vover(type) {
        ...
        if (type == "|" || type == ")") {
            ...
            if (curvoice.time != vover.p_voice.time) {
                ...
                if (curvoice.time > vover.p_voice.time) vover.p_voice.time = curvoice.time
            }
    ...
    function get_voice(parm) {
        var v, transp, vtransp, vs, a = info_split(parm, 1),
            vid = a.shift();
        if (!vid) return;
        if (vid.indexOf(",") > 0) {
            vs = vid.split(",");
        ...
        if (parse.state < 2) {
            ...
            if (vid != "*" && parse.state == 1) new_voice(vid);
            return
        }
        if (vid == "*") {
    ...
    function goto_tune(is_K) {
        ...
        curvoice = voice_tb[par_sy.top_voice];
        sym_link(s);
        if (staves_found < 0) s.default = true;
        curvoice = p_voice
    }
    ...
    function gch_build(s) {
        var gch, wh, xspc, ix, pos = curvoice.pos.gch == SL_BELOW ? -1 : 1,
            y_above = 0,
            y_below = 0,
            y_left = 0,
            y_right = 0,
        ...
        for (ix = 0; ix < s.a_gch.length; ix++) {
            ...
            if (gch.type == "g") {
                if (cfmt.chordnames) {
                    ...
                    if (cfmt.chordnames.B == "H") gch.text = gch.text.replace(/Hb/g, "Bb")
                }
                ...
                if (gch.type == "@" && !user.anno_start && !user.anno_stop) continue
            }
    ...
    function draw_gchord(s, gchy_min, gchy_max) {
        ...
        for (ix = 0; ix < s.a_gch.length; ix++) {
            ...
            if (gch.box) xy_str_b(x, y, text);
            else xy_str(x, y, text);
...
abc2svg.modules = {
    ...
    all_m: new RegExp("ambitus|beginps|break|capo|clip|voicecombine|diagram|grid2|grid|MIDI|percmap|sth", "g"),
    nreq: 0,
    hooks: [],
    g_hooks: [],
    load: function(file, relay, errmsg) {
        ...
        var m, r, nreq_i, all = file.match(this.all_m);
        if (!all) return true;
        nreq_i = this.nreq;
        ...
        for (var i = 0; i < all.length; i++) {
            ...
            r = new RegExp("(^|\\n)(%.|I:|\\[) *" + all[i] + "\\s");
            if (!r.test(file)) continue;
            m.loaded = true;
            this.nreq++;
            abc2svg.loadjs(m.fn, function() {
